  ...
  const compareRequests = useCallback(async () => {
    ...
    try {
      ...
      const axiosPromise = axios.get(url).then(res => {
        let timeAxios = parseFloat((performance.now() - axiosStart).toFixed(2));
        console.log(`Axios tomó: ${timeAxios}ms`);        
        localStorage.setItem('axiosTime', timeAxios);
        setTimeAxios(timeAxios);
      ...
      const fetchPromise = fetch(url)
        .then(async res => {
          ...
          let timeFetch = parseFloat((performance.now() - fetchStart).toFixed(2));
          console.log(`Fetch tomó: ${timeFetch}ms`);
          localStorage.setItem('fetchTime', timeFetch);
          setTimeFetch(timeFetch);
          return data.results;
        });
